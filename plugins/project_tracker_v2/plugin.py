"""
Project Tracker 2.0 — Plugin entry point.

Reuses the exact same service / repository / models as v1 (same DB tables,
existing data preserved).  Only the UI is new.
"""
...
class Plugin(PluginBase):
    plugin_id    = "project_tracker_v2"
    display_name = "Projects 2.0"
    name         = "Projects 2.0"
    version      = "2.0.0"
    description  = "Premium project tracker — milestones, sessions, gallery, requirements."
    ...
    def activate(self):
        from plugins.project_tracker.service import ProjectService
        from plugins.project_tracker.models  import ValidationError, ProjectStatus
        ...
        self.context.services.register("project_service", self._service, override=True)

        try:
            from .ui import ProjectTrackerV2UI, ProjectEditDialog
            self._ProjectEditDialog = ProjectEditDialog
            self._ui = ProjectTrackerV2UI(self.context)
        except Exception as e:
            import traceback
            log.error(f"[PROJECT V2] UI construction failed: {e}")
            traceback.print_exc()
            self._ProjectEditDialog = None
            from PySide6.QtWidgets import QLabel
            fallback = QLabel("⚠  Projects 2.0 failed to load — check console.")
        ...
        try:
            ...
            log.error(f"[PROJECT V2] Initial load failed: {e}")

        log.info("[PROJECT V2] Activated")
    ...
    def deactivate(self):
        ...
        self._ui      = None
        self._service = None
        log.info("[PROJECT V2] Deactivated")
    ...
    def _is_full_ui(self) -> bool:
        try:
            from .ui import ProjectTrackerV2UI
            return isinstance(self._ui, ProjectTrackerV2UI)
        except Exception:
            return False
    ...
    def _load_initial(self):
        ...
        try:
            ...
            stats_map = {p.id: self._service.get_stats(p.id) for p in projects}
            self._ui.display_projects(projects, stats_map=stats_map)

            all_projects = projects if not self._current_status_filter \
            ...
            import traceback
            log.error(f"[PROJECT V2] _load_initial: {e}")
            traceback.print_exc()
    ...
    def _load_project(self, project_id: int):
        ...
        try:
            ...
            self._ui.display_project_detail(
                project, stats, milestones, notes, sessions, linked,
                gallery, requirements=requirements,
            )
        except Exception as e:
            import traceback
            log.error(f"[PROJECT V2] _load_project {project_id}: {e}")
            traceback.print_exc()

    def _refresh_after_milestone(self, project_id: int):
    ...
    def _refresh_list(self, status=_SENTINEL):
        if not self._is_full_ui():
            return
        try:
            effective = self._current_status_filter if status is _SENTINEL else status
            projects  = self._service.get_all_projects(effective)
            stats_map = {p.id: self._service.get_stats(p.id) for p in projects}
            self._ui.display_projects(projects, stats_map=stats_map)
        except Exception as e:
            log.error(f"[PROJECT V2] _refresh_list: {e}")
    ...
    def _read_saved_status_filter(self):
        ...
        try:
            ...
            if not settings:
                return None
            return _MAP.get(settings.get("project_tracker.status_filter", ""))
        except Exception:
            return None
    ...
    def _register_events(self):
        ...
        self._sub("project_v2_status_changed",    self._on_quick_status)
    ...
    def _on_create(self, payload: dict):
        from plugins.project_tracker.models import ValidationError
        try:
            ...
            log.error(f"[PROJECT V2] Create: {e}")

    def _on_quick_status(self, payload: dict):
        """Handles project_v2_status_changed — V2-only event, never seen by V1."""
        ...
        try:
            ...
            self._refresh_list()
            self._load_project(project_id)
            self.context.event_bus.emit("project_updated", {"id": project_id, "status": new_status})
        except Exception as e:
            log.error(f"[PROJECT V2] Quick status: {e}")
    ...
    def _on_delete(self, payload: dict):
        ...
        try:
            ...
            log.error(f"[PROJECT V2] Delete: {e}")
    ...
    def _on_milestone_add(self, payload: dict):
        from plugins.project_tracker.models import ProjectPriority
        try:
            ...
            log.error(f"[PROJECT V2] Milestone add: {e}")
    ...
    def _on_milestone_update(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Milestone update: {e}")
    ...
    def _on_milestone_toggle(self, payload: dict):
        try:
            ...
            if m.is_complete:
                ...
                self.context.event_bus.emit("project_milestone_completed", {"id": mid, "title": m.title})
            else:
                self.context.event_bus.emit("project_milestone_uncompleted", {"id": mid, "title": m.title})
            if pid:
                self._refresh_after_milestone(pid)
        except Exception as e:
            log.error(f"[PROJECT V2] Milestone toggle: {e}")
        ...
        try:
            ...
            if milestone:
                ...
                def _undo(_m=milestone):
                    try:
                        _svc.add_milestone(project_id=_m.project_id, title=_m.title,
                                           description=_m.description or "", due_date=_m.due_date)
                        if _pid and _plug._is_full_ui():
                            _plug._refresh_after_milestone(_pid)
                    except Exception as ex:
                        log.error(f"[UNDO] Restore milestone: {ex}")

                from ui.toast import ToastManager
                from ui.undo_manager import UndoManager
                UndoManager.instance().push(f"Deleted milestone '{milestone.title}'", _undo)
                ToastManager.instance().show(
                    f"Deleted milestone '{milestone.title}'",
                    level="info", duration=6000, action_label="Undo", action_fn=_undo,
                )

            if self._current_project_id:
                self._refresh_after_milestone(self._current_project_id)
        except Exception as e:
            log.error(f"[PROJECT V2] Milestone delete: {e}")
    ...
    def _on_milestone_focus_toggle(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Milestone focus toggle: {e}")
    ...
    def _on_milestone_quantity_step(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Milestone qty step: {e}")
    ...
    def _on_note_add(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Note add: {e}")
    ...
    def _on_note_update(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Note update: {e}")
    ...
    def _on_note_delete(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Note delete: {e}")
    ...
    def _on_session_log(self, payload: dict):
        ...
        try:
            ...
            log.error(f"[PROJECT V2] Session log: {e}")
    ...
    def _on_session_start(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Session start: {e}")
    ...
    def _on_session_end(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Session end: {e}")
    ...
    def _push_session_to_calendar(self, project_id, session) -> None:
        try:
            ...
            h = session.duration_minutes // 60
            m = session.duration_minutes % 60
            notes_parts = [p for p in [session.notes, f"Outcome: {session.outcome}" if session.outcome else ""] if p]
            cal.add_event(
                title            = f"{project_icon}{project_name}  ·  {'%dh %dm' % (h,m) if h else '%dm' % m}",
            ...
            log.error(f"[PROJECT V2] Calendar push: {e}")
    ...
    def _on_session_delete(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Session delete: {e}")
    ...
    def _on_link_entity(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Link entity: {e}")
    ...
    def _on_unlink_entity(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Unlink entity: {e}")
    ...
    def _on_gallery_add(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Gallery add: {e}")
    ...
    def _on_gallery_update(self, payload: dict):
        try:
            ...
            self._service.update_gallery_entry(entry_id, **{
                ...
                if k in ("title", "note", "captured_at", "milestone_id", "session_id", "progress_stage")
            })
            if self._current_project_id:
                self._load_project(self._current_project_id)
        except Exception as e:
            log.error(f"[PROJECT V2] Gallery update: {e}")
    ...
    def _on_gallery_delete(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Gallery delete: {e}")
    ...
    def _on_requirement_add(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Req add: {e}")
    ...
    def _on_requirement_update(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Req update: {e}")
    ...
    def _on_requirement_delete(self, payload: dict):
        try:
            ...
            log.error(f"[PROJECT V2] Req delete: {e}")

    # ── Dashboard deep-link ───────────────────────────────────────────────────

    def _on_dashboard_navigate(self, payload: dict):
        if payload.get("plugin_id") not in (self.plugin_id, "project_tracker"):
            return
        if not self._is_full_ui():
            return
        try:
            ...
            log.error(f"[PROJECT V2] dashboard_navigate: {e}")
